...
ivovflo:call	t_trap			| #4: overflow int
	...
	.word	0
	.word	4
syscall:call	t_trap			| #/20: system call
	.byte		/20
	.word	_scall
	.word	/20
...
start:
	...
	mov	ax, #/0000				| Kernels DS value
	mov	ds, ax			| which is just right for DS,
	mov	es, ax			| ES, and
	mov	ss, ax			| SS.
	mov	sp, #/1000			| temp boot stack
...
|***
	...
	seg	cs
	movb	state, #	/01				| set (soft) user mode
...
| Bit 4 = 1 --> slow clock speed
  ...
  mov   dx, #/ce                  | PPI 8255 control register 
  movb  al, */94                   | PPI 8255 control word default value
  out                       | switch port A to default (=input) again  
  pop   dx


| process _haltcpu parameter
|  0  -> regular system shutdown and exit to monitor
|   1   -> reboot system
| 2 -> poweroff system into standby for remote poweron
...
pwroff:
  ...
  mov   dx, #/00cc         | PPI 8255 port C
  movb  al, */20           | set Bit 5: SET-     /e02c        -OFF
  out                       |      /e02c         power off
  .byte /f4                 | hlt 
...
_spl0:					| Everything ON
	movb	ah, #/00				| PIC mask for all enabled
	j	spl			| finish.
...
vloop:
	...
	seg	cs
	testb	#0(bp),#	/40+	/20	|must remember vector ?
	...
	movb	dl,al
istrap:	cmp	#10(bp),#/1000				|previous mode = user mode ?
	...
	j	vloop
vlexit:	cmp	#10(bp),#/1000				|previous mode = system mode ?
	je	sysmode
	seg	cs
	cmpb	state,#	/FE		
	jne	nosys
sysmode:orb	dh,#	/80		|set system mode bit in vector
nosys:	mov	#6(bp),dx		|replace old vector by compressed one
	mov	ax,ss			|get stack segment
	mov	bp, sp			| save user SP
	testb	dh,#	/80		| must switch stacks ?
	jnz	sysstk			|no
	mov	sp, #/0000				| kernel DS
...
doioin1:mov	bx, ax			|
	...
	out			/C2							| mask the new level and current levels
	xchgb	ah,al			| save old mask for return
	movb	al, #	/20				| now do EOI
	out		/C0					| mask doesn't allow more at this level
	...
	j	xtraps
xtraps:	sti
	testb	#13(bp),#	1+	2+4+	8+	/10
	jz	xsoft		|no traps pending
	call	_trap
xsoft:	testb	#13(bp),#	/20
	jz	nosint
...
_waitloc:				| so "they" know we were idle
	cli				| int's OFF while fussing mask
	pop     ax
	out     		/C2				                | restore callers PIC-mask
	movb    _Idlef,*0
...
resum1:
	...
	mov	es, ax
	mov	ds, ax
	movb	al, #/00				| want spl0()
	out			/C2						| all levels enabled
...
_clkstart:
	...
	out	/D0					| goes out first
	movb	al, _clknumb+1		| most significant byte
	out	/D0					| goes out next
...
| read/write sectors after the /20          //30           command has been issued
| blocking polling version w/o interrupts
| returns 0 on success or    1             /   2              otherwise
| in case of    2              the low-byte contains    2             
...
| read status register and wait until     /80           is cleared and     /08           is set
| we consider having     /08           not set as an error,      /20           and     /01           flags not relevant
        mov     dx, #   /e01e                       | use status register as port address
        cld                             | direction for inm/outm: address increment
sectLoop:
        mov     cx, #65535         
wReady:                                 | wait until     /80           is cleared
        in                              | read status ('in' == 'in al, dx')
        test    al, *    /80                          |     /80           cleared?
...
wRetry:
        ...
        loop    wReady 
        mov     ax, #   1                            | timeout error
        j       ioDone

dataReady:
        mov     dx, #   /e010                       | use data register as port address
...
finChk:
        mov     bl, *     /20                           | build error mask
        or      bl, *    /01                          | all bits have to be zero
        or      bl, *    /08          
        or      bl, *    /80          
        in                              | read status
...
ioERR2:
        ...
        movb    al, *   2                            | and mark as "other" error
